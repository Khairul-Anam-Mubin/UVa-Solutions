...
bool invalidcheck(string num , ll base) {
    int len = num.size() ;
    for(int i = 0 ; i < len ; i++) {
        ll x ;
        if(num[i] <= 'Z' && num[i] >= 'A')  
...
int main() {
    ...
    ll base , tobase ;
    string num ;
    while(cin >> base >> tobase >> num) {
        bool flag = invalidcheck(num , base) ;
        if(flag) {
            int len = num.size() ;
            ll dec = 0 ;
            for(int i = 0 ; i < len ; i++) {
            ...
            while(dec) {
                ...
                if(r <= 9)
                    ...
                    str.pb((char)(r - 10LL + 'A' )) ;
                dec /= tobase ;
            }
            reverse(str.begin() ,str.end()) ;
            if((int)str.size() == 0)
                str.pb('0') ;
            cout << num << " base " << base << " = " << str << " base " << tobase << "\n" ;
        }
        else {
