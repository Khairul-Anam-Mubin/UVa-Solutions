...
ll factorialDigitExtended ( ll n, ll base ) {
    double x = 0;
    for ( ll i = 1; i <= n; i++ ) {
...
ll factfactorize(ll n , vector <pair <ll , ll> > base) {
    ll ans = 100000000 ;
    ...
    while(cin >> n >> b) {
        ...
        for(ll i = 0 ; i < prime.size() && prime[i] <= sq ; i++) {
            if( x % prime[i] == 0) {
                ...
                sq = sqrt(x) ;
                base.pb(MP(prime[i] , p)) ;
            }
        }
        if( x != 1)
            base.pb(MP(x , 1LL)) ;
        ll ans = factfactorize(n , base) ;
        cout << ans << " " << factorialDigitExtended(n , b)  << "\n" ;
    }
    return 0 ;
}
