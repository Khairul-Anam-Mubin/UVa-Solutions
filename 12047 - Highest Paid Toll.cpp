...
vector < pair<int , int> > G1[10010] , G2[10010] ;
int dissource[10010] ;
int disdesti[10010] ;
int nodes , edges ;
void dijkstra(int from , int to , int maxToll,int dist[] , vector <pair <int ,int> > G[]) {
...
int main() {
	...
	while(tc--) {
		int from , to , maxToll ;
		priority_queue<pair<int,pair <int , int> > > list ; 
		scanf("%d%d%d%d%d",&nodes,&edges,&from,&to,&maxToll) ;
		for(int i = 0 ; i < edges ; i++) {
			int u , v , w ;
			scanf("%d%d%d",&u,&v,&w) ;
			list.push(MP(w , MP(u , v))) ;
			G1[u].pb(MP(w , v)) ;
			G2[v].pb(MP(w , u)) ;
		}
		dijkstra(from , to , maxToll , dissource , G1 ) ;
		dijkstra(to , from , maxToll , disdesti , G2 ) ;
		int ans = -1 ;
		while(!list.empty()) {
			int u = list.top().second.first ;
			int v = list.top().second.second ;
			int w = list.top().first ;
			list.pop() ;
			if(dissource[u] + w + disdesti[v] <= maxToll) {
				ans = w ;
				break;
			}
		}
