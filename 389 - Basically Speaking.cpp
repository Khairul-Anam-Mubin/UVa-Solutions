...
ll basetodec(string str ,ll base) {
    ll len = str.size() ;
    ll sum = 0LL ;
    for(ll i = 0LL ; i < len ; i++) {
        if(str[i] >= '0' && str[i] <= '9')
            sum += Pow(base , len - i - 1LL) * (str[i] - '0') ;
        else if(str[i] >= 'A' && str[i] <= 'Z')
            sum += Pow(base , len - i - 1LL) * (str[i] - 'A' + 10LL) ;
    }
    return sum ;
}
string dectobase(ll num , ll tobase) {
    string str ;
    while(num) {
        ll rem = num % tobase ;
        if(rem >= 0LL && rem <= 9LL)
...
int main() {
    ...
    string num ;
    ll base , tobase ;
    while(cin >> num >> base >> tobase) {
        ll dec = basetodec(num , base) ;
        string str = dectobase(dec , tobase) ; 
        cout << str << "\n" ;
    }
    return 0 ;
}
