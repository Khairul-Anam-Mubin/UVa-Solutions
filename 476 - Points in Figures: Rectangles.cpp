...
double DistanceOfCartesianPoints(point a , point b) {
    return sqrt((Sqr(a.x - b.x) + Sqr(a.y - b.y)));
}
double RectangleAreaByPoints(point a, point b, point c, point d) {
    double ab = DistanceOfCartesianPoints(a , b);
    double bc = DistanceOfCartesianPoints(b , c);
    double cd = DistanceOfCartesianPoints(c , d);
    double area;
    if (Abs(ab - bc) > eps) {
        area = ab * bc;
    } else {
...
double TriangleAreaByPoints(point a, point b , point c) {
    double ab = DistanceOfCartesianPoints(a , b);
    double bc = DistanceOfCartesianPoints(b , c);
    double ac = DistanceOfCartesianPoints(c , a);
    double perimeter = (ab + bc + ac);
    double half_perimeter = perimeter * 0.5;
    double area = half_perimeter * (half_perimeter - ab) * (half_perimeter - bc) * (half_perimeter - ac);
    area = sqrt(area);
    return area;
}
int main() {
    FasterIO
    char ch;
    vector <pair <pair <point , point> , pair <point , point>>> v;
    vector <double> recarea;
    while (cin >> ch && ch != '*') {
        double x1 , y1 , x3 , y3;
        cin >> x1 >> y1 >> x3 >> y3;
        double x2 , y2, x4 , y4;
        x2 = x1 , y2 = y3;
        x4 = x3 , y4 = y1;
        v.emplace_back(make_pair(make_pair(point(x1 , y1) , point(x2 , y2)) , make_pair(point(x3 , y3) , point(x4 , y4))));
    } 
    for (int i = 0; i < v.size(); i++) {
        recarea.emplace_back(RectangleAreaByPoints(v[i].F.F , v[i].F.S, v[i].S.F , v[i].S.S));
    }
    double x , y;
    int k = 1;
    while(cin >> x >> y) {
        ...
        bool flag = 1;
        for (int i = 0; i < v.size(); i++) {
            double one = TriangleAreaByPoints(v[i].F.F , v[i].F.S , cur);
            double two = TriangleAreaByPoints(v[i].F.S , v[i].S.F , cur);
            double three = TriangleAreaByPoints(v[i].S.F , v[i].S.S , cur);
            double four = TriangleAreaByPoints(v[i].S.S , v[i].F.F , cur);
            double area = one + two + three + four;
            if (Abs(recarea[i] - area) < eps) {
