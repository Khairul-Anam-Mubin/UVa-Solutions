...
{
    double red  = ((deg * pi)/180.0);
    return red;
}
double mintored(double mint)
{
    double deg = (mint / 60.0);
    double red = degtored(deg);
    return red;
}
int main()
{
    ...
    while(cin >> r >> red >> str)
    {
        if(str == "deg")
            ...
            red = mintored(red);
        red = min(red,2.0*pi-red);
        r = r + (double)6440.0;

        double s = r * red ;
        double e = sqrt((2.0 * Sqr(r)) - ((2.0 * Sqr(r)) * cos(red)));
